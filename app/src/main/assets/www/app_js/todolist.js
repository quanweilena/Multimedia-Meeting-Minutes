TODOLIST = (function() {
        ...
        for (var ii = 0; ii < currentTasks.length; ii++) {
            ...
            listHtml += "<li id='task_" + currentTasks[ii].id + "'>" + dueDateHtml +
                ...
                "<div class='task-details'>" + 
                    ...
                    "<a href='#' class='task-complete right'>Delete Reminder</a>&nbsp;" + 
                "</div>" +
                "<div class='task-index'>" + ii + "</div>" + 
                "</li>";
        } // for
    ...
    function toggleDetailsDisplay(listItem) {
        ...
        activeItem = listItem;
        currentTaskIndex = jQuery(listItem).find(".task-index").text();
    ...
    var module = {
        ...
        Storage: (function() {
            ...
            try {
                ...
                db.changeVersion("1.0", "1.2", function(transaction) {
        ...
        activateAllTasks: function() {
            TODOLIST.Storage.getTasksInPriorityOrder(function(tasks) {
                ...
                jQuery("ul#tasklist a.task-complete").unbind().click(function() {
                    // complete the task
                    alert("complete the task");
                    //the following handle actual delete task
                	//alert("current name " + currentTasks[currentTaskIndex].name);
                	//alert("current description " + currentTasks[currentTaskIndex].description);
                	//alert("current due " + currentTasks[currentTaskIndex].due);
                	//alert("current id " + currentTasks[currentTaskIndex].id);
                	//alert("current completed before call complete " + currentTasks[currentTaskIndex].completed);
                	currentTasks[currentTaskIndex].complete();
                	//alert("current completed after call complete " + currentTasks[currentTaskIndex].completed);
                    TODOLIST.Storage.saveTask(currentTasks[currentTaskIndex], module.activateAllTasks);
                    populateTaskList();
                });
